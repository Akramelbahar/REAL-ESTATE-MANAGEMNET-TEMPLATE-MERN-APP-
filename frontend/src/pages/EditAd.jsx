...
function EditAd() {
  ...
  useEffect(() => {
    axios.get(`https://backend-hgsc.onrender.com/api/user/listing/${AdId}`, {
      headers: {
        token: authToken
      }
    })
    ...
    <>
      ...
      <div className='relative xl:mx-12 xl:my-4 lg:mx-8 lg:my-4 md:mx-6 md:my-3 sm:mx-8 sm:my-2 my-2 mx-4 min-h-screen rounded-md shadow-xl ring p-4 bg-base-300'>
        <div role="alert" className="alert m-3 space-y-2 block">
          <h3 className='block text-xl dark:text-white text-bold font-bold'>Modifier Votre Annonce :</h3>
        ...
        <div className='md:w-3/5 w-full mx-auto'>
          {TypeBien === "Appartement" || TypeBien === "Maison" ? (
            <select defaultValue={TypeBien} onChange={(e) => setTypeBien(e.target.value)} className="input w-4/6 md:w-2/6 input-bordered">
