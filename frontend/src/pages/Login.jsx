    ...
    const submitData = async (e) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                setIsLoggedIn(true)
                setResponseMessage(`User: ${userId}`); 
                setAlertType("alert-success alert_display");
            } else {
                setResponseMessage(JSON.stringify(response.data, null, 2));
                setAlertType("alert-warning alert_display");
            }
        } catch (error) {
            console.error("Error:", error);
            setAlertType("alert-error alert_display");
            if (error.response && error.response.data) {
                setResponseMessage(JSON.stringify(error.response.data, null, 2)); 
                if (error.response.status === 409) setAlertType("alert-warning alert_display");
            } else {
                setResponseMessage("Message: We encountered some errors. Please check your inputs and try again.");
            }
        } finally {
            setTimeout(() => {
                setAlertType("alert_hidden");
            }, 6000);
        }
    };
                ...
                <div className='container p-2 mx-auto relative'>
                    ...
                    <form onSubmit={submitData} className='my-7'>
                        ...
                        <div className="mx-auto w-3/4">
                            <label className="input input-bordered flex items-center gap-2 my-6 w-1/2 mx-auto h-16">
                        ...
                        <button className="btn btn-active btn-primary block my-3 mx-auto">Se connecter</button>
                    </form>
                </div>
                <Footer />
            </>
